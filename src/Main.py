...
elevator_motor  = Motor(Ports.PORT17, GearSetting.RATIO_18_1, False)
...
def zero_elevator():
    elevator_motor.spin_to_position(360, DEGREES, 100, PERCENT)
    print("Started zeroing elevator")
    while(elevator_limit_switch.pressing() == True):
        elevator_motor.spin(REVERSE, 200)
    else:
        elevator_motor.stop()
...
while True:
    ...
    if state == STANDBY:
        ...
        if controller.buttonY.pressing():
            elevator_motor.spin(FORWARD, 200)
