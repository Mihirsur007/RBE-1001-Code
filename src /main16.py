...
while True:
    ...
    # Auto mode = runs a program and stops after some time ----
        ...
        if controller.buttonY.pressing():
            ...
            if(get_tilt() < 10):
                if a_ramp_tilt_mode == 2:
                   a_ramp_tilt_mode = 3
                a_ramp_rot = rotate(135, 0.8)
                a_ramp_Y = -20
                
            if(get_tilt() >= 10) and (a_ramp_tilt_mode != 4):
                if a_ramp_tilt_mode != 2:
                    a_ramp_tilt_mode = 1
                a_ramp_rot = rotate(180, 0.8)
                a_ramp_Y = -50
        ...
        current_heading = inertial.heading()
        m_rot = a_ramp_rot + m_ato_rot + m_gtb_atat_rot

        drive_x(0, a_ramp_Y, (m_ato_strafe_X + m_gtb_atat_strafe_X + a_gtb_atat_strafe_X), (m_ato_strafe_Y + m_gtb_atat_strafe_Y + a_gtb_atat_strafe_Y), (a_ramp_rot + m_ato_rot + m_gtb_atat_rot), current_heading)
