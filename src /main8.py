...
def allign_to_object():
    ...
    if (target_fruit is not None) and (target_fruit.width > 20):
        ...
        if not (abs(error_X) < 10) or (target_fruit.centerX < EDGE_MARGIN or target_fruit.centerX > (320 - EDGE_MARGIN)) or (abs(rot) > 1.5):
...
def allign_to_apriltag(tag):
    ...
    error_heading = 0 - current_heading
...
while True:
    ...
    # Move mode = teleop --------------------------------------
        ...
        if (controller.buttonB.pressing()):
            ...
            if has_fruit == True:
                hf_timer += 1
                if (hf_timer > 200) and (hf_timer < 300):
